...
class CRM_Social_OAuthProvider {
  /**
  ...
  public function getCallbackUri($oAuthProvider) {
    return CRM_Utils_System::url("civicrm/social/callback/{$oAuthProvider}", NULL, TRUE, NULL, FALSE);
  }
  ...
  public function saveSocialUser($oAuthProvider, $userProfile, $accessToken) {
    ...
    if (!$contactId) {
      ...
      $contactId = civicrm_api3('SocialUser', 'createcontact', $user);
      ...
      civicrm_api3('SocialUser', 'create', $socialUser);
    }
  ...
  public function login($oAuthProvider = NULL, $oAuthProviderId = NULL, $contactId = NULL) {
    ...
    if (($oAuthProvider == NULL && $oAuthProviderId == NULL && $contactId == NULL)
      ...
      $session->set('social_oauth_provider', $oAuthProvider);
      $session->set('social_social_user_id', $oAuthProviderId);
      $session->set('social_contact_id', $contactId);
      return TRUE;
    }
    return FALSE;
  }
  ...
  public function isLoggedIn() {
    $session = CRM_Core_Session::singleton();
    $oAuthProvider = $session->get('social_oauth_provider');
    if ($oAuthProvider) {
  ...
  public function getSkipLogin() {
    $session = CRM_Core_Session::singleton();
    $skipLogin = $session->get('social_skip_login');
    return $skipLogin;
  }
  ...
  public function setSkipLogin($skipLogin = NULL) {
    $session = CRM_Core_Session::singleton();
    $session->set('social_skip_login', $skipLogin);
  }
  ...
  public function redirect($accessDenied = FALSE) {
    if (isset($_GET['continue'])) {
      ...
      $session = CRM_Core_Session::singleton();
      $continueUrl = $session->get("social_redirect");
      $session->set('social_redirect', NULL);
      if (!$continueUrl) {
  ...
  public function saveRedirect($redirectUrl) {
    $session = CRM_Core_Session::singleton();
    $session->set('social_redirect', $redirectUrl);
  }
