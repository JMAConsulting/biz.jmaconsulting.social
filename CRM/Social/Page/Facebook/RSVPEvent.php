 ...
 */
class CRM_Social_Page_Facebook_RSVPEvent extends CRM_Core_Page {
  ...
  public function run() {
    ...
    if (!isset($_GET['event_id']) || !$oap->isLoggedIn() || 'facebook' != $session->get('social_oauth_provider')) {
      $this->redirect();
    }
    ...
    $facebook = new CRM_Social_OAuthProvider_Facebook($session->get('facebook_access_token'));
    ...
    if (!empty($deniedPermissions)) {
      ...
      if ($rsvpRequested) {
        ...
        $currentUrl = CRM_Utils_System::url("civicrm/social/event/rsvpfacebookevent?event_id={$eventId}", NULL, TRUE);
        $session->set('social_redirect', $currentUrl);
